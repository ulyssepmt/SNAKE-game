...
void gameLogic(struct SNAKE* pSnake, struct FOOD* pFood, HDC hdc, 
    ...
    while (TRUE) {
        ...
        if ((GetKeyState(VK_RIGHT) & 0x8000) && blockDir != 2) { dir = 0; }

        if ((GetKeyState(VK_DOWN) & 0x8000) && blockDir != 3) { dir = 1;  }
   
        if ((GetKeyState(VK_LEFT) & 0x8000) && blockDir != 0) { dir = 2;  }

        if ((GetKeyState(VK_UP) & 0x8000) && blockDir != 1) { dir = 3;   }
